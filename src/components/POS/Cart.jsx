...
const Cart = ({ cart, selectedWashPay, onRemoveWashPay, onClearCart, itbisRate, fiscalData, setFiscalData, paymentMethod, setPaymentMethod, onCheckout }) => {
  const getCartTotal = () => {
    ...
    <div className="bg-luxury-dark rounded-[3rem] p-10 text-white shadow-2xl relative overflow-hidden">
      ...
      {selectedWashPay && (
        <div className="mb-6 p-4 bg-blue-500/20 border border-blue-400/30 rounded-2xl relative">
          <button onClick={onRemoveWashPay} className="absolute top-2 right-2 text-white/50 hover:text-white">×</button>
      ...
      <div className="pt-8 border-t border-white/5 space-y-6">
        ...
        <div className="space-y-1">
          ...
          >
            ...
            {selectedWashPay?.company_id && (
              <option value="Crédito Flotilla" className="bg-luxury-dark text-emerald-400 font-bold font-black">🏢 Crédito Flotilla</option>
            )}
          </select>
        </div>
        ...
        <div className="space-y-2 mb-6 border-b border-white/10 pb-6">
          <div className="flex justify-between text-[10px] font-bold uppercase tracking-widest opacity-40">
            <span>Subtotal</span>
            <span>RD${total.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
          </div>
        ...
        <div className="flex justify-between items-end mb-8">
          ...
          <h4 className="text-4xl font-black tracking-tighter">RD${finalTotal.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</h4>
        </div>

        <button
          onClick={() => onCheckout(paymentMethod, selectedWashPay)}
        ...
        {cart.length > 0 && (
          <button onClick={onClearCart} className="w-full text-[10px] font-black opacity-20 hover:opacity-50 uppercase tracking-tighter">Limpiar Todo</button>
        )}
      </div>
    </div>
  );
